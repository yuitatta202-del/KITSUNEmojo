...
// Services & Types
import { AdminAction } from './admin.service';
import {
  SupabaseService,
  Manga,
  MangaUpdate,
...
export class AdminController {
  ...
  constructor(
    ...
    if (!supabaseService) {
      throw new Error('SupabaseService non disponibile');
    }
    if (!importService) {
      throw new Error('ImportService non disponibile');
    }
    if (!eventEmitter) {
      throw new Error('EventEmitter2 non disponibile');
    }
    if (!cacheManager) {
      this.logger.warn(
        'CacheManager non disponibile - il servizio funzionerà senza cache',
      );
    }
  ...
  async getAllArtists(
    ...
    try {
      ...
      if (includeStats === 'true') {
        this.logger.debug(
          `[${requestId}] Arricchimento statistiche per ${result.length} artisti`,
        );

        const enrichedResults = await Promise.allSettled(
          result.map(async (artist): Promise<ArtistResponse> => {
            try {
              ...
              if (error) {
                this.logger.warn(
                  `[${requestId}] Errore conteggio manga per artista ${artist.id}: ${error.message}`,
                );
              ...
              this.logger.warn(
                `[${requestId}] Eccezione conteggio per artista ${artist.id}: ${this.getErrorMessage(err)}`,
              );
              return { ...artist, mangaCount: 0 };
            }
          }),
        ...
        result = enrichedResults
          ...
          .map((r) => (r as PromiseFulfilledResult<ArtistResponse>).value);
      }

      await this.cacheManager.set(cacheKey, result, 300000);
      this.logger.log(`[${requestId}] ✅ Recuperati ${result.length} artisti`);
      ...
      throw new HttpException(
        {
          code: 'ARTISTS_FETCH_FAILED',
          message: 'Impossibile recuperare la lista degli artisti',
  ...
  async getAllTags(
    ...
    try {
      ...
      if (includeUsage === 'true' && result.length > 0) {
        ...
        if (error) {
          this.logger.warn(`Errore recupero usage tags: ${error.message}`);
        } else if (data) {
      ...
      const errorMessage = this.getErrorMessage(err);
      this.logger.error(
        `[AdminController] Errore recupero tags: ${errorMessage}`,
      );

      throw new HttpException(
        'Impossibile recuperare la lista dei tags',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getAllCategories(): Promise<CategoryResponse[]> {
    try {
      ...
      const errorMessage = this.getErrorMessage(err);
      this.logger.error('Errore recupero categorie', errorMessage);
      throw new HttpException(
  ...
  async getSystemStats(): Promise<SystemStatsResponse> {
    try {
      ...
      const errorMessage = this.getErrorMessage(err);
      this.logger.error('Errore recupero stats', errorMessage);
      throw new HttpException(
  ...
  async getPopularTags(
    ...
    try {
      ...
      const errorMessage = this.getErrorMessage(err);
      this.logger.error('Errore recupero tag popolari', errorMessage);
      throw new HttpException(
  ...
  async getStats(
    ...
    try {
      ...
      const errorMessage = this.getErrorMessage(err);
      this.logger.error('Errore recupero statistiche', errorMessage);
